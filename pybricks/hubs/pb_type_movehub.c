...
STATIC mp_obj_t hubs_MoveHub_IMU_tilt(size_t n_args, const mp_obj_t *pos_args, mp_map_t *kw_args) {
    ...
    uint8_t index[3];
    int8_t sign[3];
    pbio_orientation_side_get_axis(pb_type_enum_get_value(up_in, &pb_enum_type_Side), &index[2], &sign[2]);
    pbio_orientation_side_get_axis(pb_type_enum_get_value(forward_in, &pb_enum_type_Side), &index[0], &sign[0]);
    ...
    pbio_orientation_get_complementary_axis(index, sign);
    ...
    fix16_t accel[3];
    for (uint8_t i = 0; i < 3; i++) {
        accel[index[i]] = fix16_from_int(data[i] * sign[index[i]]);
    }
    ...
    // Pitch
    tilt[0] = mp_obj_new_int(
        fix16_rad_to_deg(
            fix16_atan2(
                -accel[0],
                fix16_sqrt(fix16_add(fix16_sq(accel[2]), fix16_sq(accel[1])))
                )
            )
        );

    // Roll
    tilt[1] = mp_obj_new_int(
        fix16_rad_to_deg(fix16_atan2(accel[1], accel[2]))
        );
